    ...
    >
        ...
        {triagens.length === 0 ? (
          ...
          <div className="overflow-x-auto">
              ...
              <thead>
                <tr className="bg-brand-floral/50 border-b border-brand-indigo/5">
                  {["Data Coleta", "Total", "Reforma", "Remanuf.", "Compra", "Sucata", "Status", "Ações"].map(h => (
                    <th key={h} className="px-5 py-4 text-[10px] font-black uppercase tracking-widest text-brand-indigo/50">{h}</th>
                  ))}
                </tr>
              </thead>
                ...
                <AnimatePresence>
                  {triagens.map((t, i) => (
                    <motion.tr key={t.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                      transition={{ delay: i * 0.02 }} className="hover:bg-brand-floral/30 transition-colors group">
                      <td className="px-5 py-4">
                        <p className="text-xs font-bold text-brand-indigo">{fmt(t.data_coleta)}</p>
                        {t.nf_saida_pce && <p className="text-[10px] text-brand-indigo/40 font-bold uppercase tracking-widest mt-0.5">NF: {t.nf_saida_pce}</p>}
                      </td>
                      <td className="px-5 py-4 text-sm font-black text-brand-indigo">{t.quantidade_total}<span className="text-[10px] text-brand-indigo/40 font-bold uppercase tracking-widest ml-1">un</span></td>
                      <td className="px-5 py-4"><span className="text-xs font-bold text-brand-orange">{t.quantidade_manutencao ?? 0}</span></td>
                      <td className="px-5 py-4"><span className="text-xs font-bold text-brand-indigo/70">{t.quantidade_remanufatura ?? 0}</span></td>
            ...
            <div className="px-5 py-4 border-t border-brand-indigo/5 bg-brand-floral/30">
              <p className="text-[10px] font-bold text-brand-indigo/40 uppercase tracking-widest">{triagens.length} registros</p>
            </div>
          </div>
        )}
        </AppCard>
        ...
        {isModalOpen && editing && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="absolute inset-0 bg-brand-indigo/30 backdrop-blur-sm" onClick={closeModal} />

            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="relative bg-white w-full max-w-2xl rounded-3xl shadow-2xl border border-brand-indigo/10 overflow-hidden max-h-[90vh] flex flex-col">

              {/* Modal Header */}
              <div className="px-7 py-5 border-b border-brand-indigo/5 flex justify-between items-center bg-white sticky top-0 z-10">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-bold text-base leading-tight text-brand-indigo">
                      {isFinalizada ? "Triagem Concluída" : "Classificação da Carga"}
                    </h3>
                    <p className="text-[10px] font-bold text-brand-indigo/50 uppercase tracking-widest mt-0.5">
                      {fmt(editing.data_coleta)} · {editing.quantidade_total} pallets
                      {editing.nf_saida_pce ? ` · NF ${editing.nf_saida_pce}` : ""}
                    </p>
                  </div>
                </div>
                <button onClick={closeModal} className="text-brand-indigo/30 hover:text-brand-indigo transition-colors"><X size={20} /></button>
              </div>

              <div className="overflow-y-auto flex-1">
                {/* Dados da coleta */}
                <div className="px-7 pt-5 pb-4 bg-brand-floral/30 border-b border-brand-indigo/5">
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      ...
                      <div key={item.label} className="bg-white rounded-2xl p-4 border border-brand-indigo/5">
                        <div className="flex items-center gap-2 text-brand-indigo/40 mb-2">{item.icon}<span className="text-[9px] font-bold uppercase tracking-widest">{item.label}</span></div>
                        <p className="text-xs font-black text-brand-indigo">{item.val}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="px-7 pt-5 pb-3">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-xs font-black text-brand-aqua">{pct.toFixed(0)}% classificado</span>
                  ...
                  <div className="w-full h-3 bg-brand-indigo/5 rounded-full overflow-hidden">
                    <motion.div animate={{ width: `${pct}%` }}
                      className={`h-full rounded-full ${saldo < 0 ? "bg-red-500" : "bg-brand-aqua"} transition-all`} />
                  </div>
                </div>
                ...
                <div className="px-7 pb-7 space-y-5">
                  {/* Totais gerais */}
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-brand-indigo/40 mb-3">Classificação Geral</p>
                      ...
                      {[
                        ...
                        <div key={f.label} className="space-y-1.5">
                          ...
                          <input type="number" min="0" disabled={isFinalizada}
                            ...
                            className={`w-full px-4 py-3 bg-white border border-brand-indigo/10 rounded-2xl text-sm font-bold outline-none focus:ring-2 focus:ring-brand-orange/20 disabled:opacity-60 text-brand-indigo`} />
                        </div>
                      ))}
                    </div>
                    <p className="text-[10px] text-brand-indigo/40 mt-3 ml-1">
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-3">
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] text-brand-indigo/40">Por Modelo (Opcional)</p>
                      {!isFinalizada && (
                    ...
                    {itensForm.length === 0 ? (
                      <div className="py-8 text-center border-2 border-dashed border-brand-indigo/10 rounded-3xl">
                        <p className="text-[11px] font-bold text-brand-indigo/30 uppercase tracking-widest">Nenhum modelo adicionado</p>
                      </div>
                    ) : (
                      <div className="space-y-3">
                        {itensForm.map((item, idx) => (
                          <div key={idx} className="bg-brand-floral/30 rounded-3xl border border-brand-indigo/5 p-5">
                            <div className="flex flex-col sm:flex-row gap-4">
                              <div className="flex-1">
                                <label className="text-[9px] font-bold uppercase tracking-widest text-brand-indigo/40 mb-1.5 block">Modelo</label>
                                <div className="relative">
                                  <select disabled={isFinalizada} value={item.modelo_pallet_id}
                                    onChange={e => changeItemModel(idx, e.target.value)}
                                    className="w-full appearance-none px-4 py-3 bg-white border border-brand-indigo/10 rounded-2xl text-xs font-bold outline-none focus:ring-2 focus:ring-brand-orange/20 disabled:opacity-60 text-brand-indigo">
                                    {initialModelosPallets.map(m => (
                                      <option key={m.id} value={m.id}>{m.codigo ? `[${m.codigo}] ` : ""}{m.nome}</option>
                                    ))}
                                  </select>
                                  <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 text-brand-indigo/30 pointer-events-none" size={14} />
                                </div>
                              </div>
                              <div className="grid grid-cols-3 gap-3 flex-[1.5]">
                                {[
                                  ...
                                  { label: "Remanuf.", field: "quantidade_remanufatura", val: item.quantidade_remanufatura, cl: "text-brand-indigo/70" },
                                  { label: "Compra", field: "quantidade_compra_ivani", val: item.quantidade_compra_ivani, cl: "text-brand-aqua" },
                                ].map(f => (
                                  <div key={f.field}>
                                    ...
                                    <input type="number" min="0" disabled={isFinalizada}
                                      ...
                                      className="w-full px-3 py-3 bg-white border border-brand-indigo/10 rounded-2xl text-xs font-bold outline-none focus:ring-2 focus:ring-brand-orange/20 disabled:opacity-60 text-brand-indigo" />
                                  </div>
                                ))}
                              </div>
                              {!isFinalizada && (
                                <button onClick={() => removeItem(idx)}
                                  className="self-end p-3 text-brand-indigo/30 hover:text-red-500 hover:bg-red-50 rounded-2xl transition-all mb-0.5">
                                  <Trash2 size={16} />
                                </button>
                              )}
                            </div>
                  ...
                  {/* Observação */}
                  <div>
                    <label className="text-[10px] font-bold uppercase tracking-widest text-brand-indigo/40 ml-1 mb-2 block">Observações</label>
                    <textarea disabled={isFinalizada} value={obs} onChange={e => setObs(e.target.value)} rows={2}
                      placeholder="Notas sobre a carga..."
                      className="w-full px-4 py-4 bg-brand-floral/30 border border-brand-indigo/10 rounded-2xl text-sm outline-none resize-none focus:ring-2 focus:ring-brand-orange/20 disabled:opacity-60 text-brand-indigo" />
                  </div>

                  {/* Ações */}
                  <div className="flex gap-3">
                    {isFinalizada ? (
                      <div className="w-full py-4 bg-brand-indigo/5 text-brand-indigo/50 rounded-2xl text-xs font-bold flex items-center justify-center gap-2">

...
interface AdminTriagemClientProps {
  initialTriagens: Triagem[];
  initialModelosPallets: ModeloPallet[];
}

export function AdminTriagemClient({ initialTriagens, initialModelosPallets }: AdminTriagemClientProps) {
  const supabase = createClient();
  const [triagens, setTriagens] = useState<Triagem[]>(initialTriagens);
  const [modelosPallets, setModelosPallets] = useState<ModeloPallet[]>(initialModelosPallets);
  ...
  const fetchTriagens = async () => {
    // Keep fetch for refetching after updates
    try {
      ...
      if (itensError) {
        console.error("Erro ao buscar itens das triagens:", itensError);
      }

      const triagensComItens = triagensData.map(triagem => {
        const itensDestaTriagem = itensData ? itensData.filter(item => item.triagem_id === triagem.id) : [];
        return {
          ...triagem,
      ...
      setTriagens(triagensComItens);
    } catch (err: any) {
      console.error("Exceção ao buscar triagens:", err);

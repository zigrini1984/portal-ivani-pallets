    ...
    >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
          <AppCard className="p-4 flex items-center gap-4">
             ...
             <div>
               <p className="text-[10px] font-black uppercase tracking-widest text-brand-indigo/60">Total Reforma</p>
               <p className="text-2xl font-black text-brand-indigo">{stats.reforma} <span className="text-[10px] font-bold text-brand-indigo/40 uppercase">un</span></p>
             </div>
          </AppCard>
          <AppCard className="p-4 flex items-center gap-4">
             <div className="p-3 rounded-2xl bg-brand-indigo/5 text-brand-indigo"><Wrench size={20} /></div>
             <div>
               <p className="text-[10px] font-black uppercase tracking-widest text-brand-indigo/60">Total Remanufatura</p>
               <p className="text-2xl font-black text-brand-indigo">{stats.remanufatura} <span className="text-[10px] font-bold text-brand-indigo/40 uppercase">un</span></p>
             </div>
          </AppCard>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <div className="relative flex-1 group">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-brand-indigo/30 group-focus-within:text-brand-orange transition-colors" size={20} />
            <input type="text" placeholder="Buscar modelo de pallet..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
              className="w-full pl-12 pr-4 py-4 bg-white border border-brand-indigo/10 rounded-2xl text-sm outline-none focus:ring-4 focus:ring-brand-orange/10 transition-all shadow-sm font-medium text-brand-indigo" />
          </div>
          
          <div className="relative min-w-[200px]">
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
              className="w-full appearance-none pl-4 pr-12 py-4 bg-white border border-brand-indigo/10 rounded-2xl text-sm font-bold outline-none cursor-pointer focus:border-brand-orange shadow-sm transition-all text-brand-indigo">
              <option value="todos">Todos Status</option>
              <option value="pendente">Pendentes</option>
              <option value="em_andamento">Iniciados</option>
              <option value="concluida">Concluídos</option>
            </select>
            <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 text-brand-indigo/30 pointer-events-none" size={18} />
          </div>
        </div>
              ...
              <table className="w-full text-left border-collapse min-w-[800px]">
                <thead>
                  <tr className="bg-brand-floral/50 border-b border-brand-indigo/5">
                    <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-brand-indigo/50">Modelo / Origem</th>
                    <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-brand-indigo/50 text-center">Quantidade</th>
                    <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-brand-indigo/50">Status / Tipo</th>
                    <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-brand-indigo/50 text-right">Ações Operacionais</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-brand-indigo/5">
                  <AnimatePresence>
                    {filteredList.map((item) => (
                      <motion.tr key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-brand-floral/30 transition-colors group">
                        <td className="px-6 py-5">
                          <div className="flex items-center gap-4">
                            <div className="w-12 h-12 rounded-2xl bg-brand-aqua/10 flex items-center justify-center text-brand-aqua group-hover:scale-110 transition-transform">
                              <Package size={24} />
                            </div>
                            <div>
                              <p className="text-sm font-black text-brand-indigo">{item.modelo_nome_snapshot || "Modelo não informado"}</p>
                              <p className="text-[10px] text-brand-indigo/50 font-bold uppercase tracking-widest mt-0.5">Triagem #{item.triagem_id?.split('-')[0]}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-5 text-center">
                          <span className="text-xl font-black text-brand-indigo">{item.quantidade}</span>
                          <span className="text-[10px] font-bold text-brand-indigo/30 ml-1.5 uppercase tracking-widest">un</span>
                        </td>
                        <td className="px-6 py-5">
                        ...
                        <td className="px-6 py-5 text-right">
                          <div className="flex justify-end gap-2">
                            ...
                            {item.status === "concluida" && (
                              <div className="flex items-center gap-2 px-4 py-3 bg-brand-indigo/5 text-brand-indigo/40 rounded-2xl text-[10px] font-black uppercase tracking-widest">

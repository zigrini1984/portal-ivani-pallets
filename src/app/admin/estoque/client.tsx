  ...
  return (
    ...
    >
        <div className="flex flex-col md:flex-row justify-end items-start md:items-center gap-4 mb-8">
          <div className="flex gap-2 bg-white p-1 rounded-2xl border border-[#133020]/10 shadow-sm w-full md:w-auto">
            <button 
              onClick={() => setActiveTab('cards')}
              className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-xs font-bold transition-all ${activeTab === 'cards' ? 'bg-[#327039] text-[#133020] shadow-lg shadow-[#327039]/20' : 'text-[#133020]/40 hover:bg-[#F8EDD9]/50'}`}
            >
              <TrendingUp size={16} /> Visão Geral
            </button>
            <button 
              onClick={() => setActiveTab('historico')}
              className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-xs font-bold transition-all ${activeTab === 'historico' ? 'bg-[#327039] text-[#133020] shadow-lg shadow-[#327039]/20' : 'text-[#133020]/40 hover:bg-[#F8EDD9]/50'}`}
            >
        ...
        {loading ? (
          <div className="py-20 flex flex-col items-center justify-center bg-white rounded-[2rem] border border-[#133020]/10">
             <Loader2 className="animate-spin text-[#327039] mb-4" size={48} />
             <h3 className="text-lg font-bold text-[#133020]">Calculando Saldos</h3>
                ...
                {estoque.map((item) => (
                  <div key={item.id} className="bg-white rounded-3xl border border-[#133020]/10 p-6 shadow-sm relative overflow-hidden group hover:shadow-md transition-shadow">
                    <div className="flex justify-between items-start mb-6">
                      <div className="w-12 h-12 bg-[#F8EDD9]/50 rounded-2xl flex items-center justify-center text-[#133020]/30 group-hover:text-[#327039] transition-colors">
                        <Package size={24} />
                      </div>
                      <button 
                    ...
                    <div className="mb-6">
                      <h3 className="text-lg font-black text-[#133020] leading-tight mb-1">{item.modelo_pallet?.nome}</h3>
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-black text-[#133020]/40 uppercase tracking-widest">{item.modelo_pallet?.codigo}</span>
                        <div className="w-1 h-1 bg-[#133020]/20 rounded-full" />
                        <span className="text-[10px] font-black text-[#133020]/40 uppercase tracking-widest">{item.modelo_pallet?.medidas}</span>
                      </div>
                    </div>
                    ...
                    <table className="w-full min-w-[650px] text-left">
                      <thead>
                        <tr className="bg-[#F8EDD9]/50 border-b border-[#133020]/5">
                          <th className="px-6 py-4 text-[10px] font-black text-[#133020]/50 uppercase tracking-widest">Data</th>
                          <th className="px-6 py-4 text-[10px] font-black text-[#133020]/50 uppercase tracking-widest">Modelo</th>
                          <th className="px-6 py-4 text-[10px] font-black text-[#133020]/50 uppercase tracking-widest">Tipo</th>
                          <th className="px-6 py-4 text-[10px] font-black text-[#133020]/50 uppercase tracking-widest text-right">Qtd</th>
                          <th className="px-6 py-4 text-[10px] font-black text-[#133020]/50 uppercase tracking-widest">Descrição</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-[#133020]/5">
                        {movimentacoes.map((mov) => (
                          <tr key={mov.id} className="hover:bg-[#F8EDD9]/30 transition-colors group">
                            <td className="px-6 py-4">
                              <span className="text-xs font-bold text-[#133020]/60">{new Date(mov.created_at).toLocaleString('pt-BR')}</span>
                            </td>
                            <td className="px-6 py-4">
                              <span className="text-xs font-black text-[#133020]">{mov.modelo_pallet?.nome}</span>
                            </td>
                            <td className="px-6 py-4">
                               <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border
                                 ${mov.tipo === 'entrada' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : mov.tipo === 'saida' ? 'bg-red-50 text-red-600 border-red-100' : 'bg-[#F8EDD9] text-[#133020]/60 border-[#133020]/10'}
                               `}>
                            ...
                            <td className="px-6 py-4">
                              <span className="text-[10px] font-medium text-[#133020]/40 italic">{mov.descricao}</span>
                            </td>
                          </tr>
                        ))}
                      </tbody>
      ...
      <AnimatePresence>
        {isOutflowModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 overflow-y-auto">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsOutflowModalOpen(false)} className="absolute inset-0 bg-[#133020]/30 backdrop-blur-sm" />
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative bg-white w-full max-w-xl rounded-3xl shadow-2xl border border-[#133020]/10 overflow-hidden" >
              <div className="px-8 py-6 border-b border-[#133020]/5 flex justify-between items-center bg-white sticky top-0 z-10">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-bold text-lg leading-tight text-[#133020]">Registrar Saída de Estoque</h3>
                    <p className="text-[10px] font-bold text-[#133020]/40 uppercase tracking-widest mt-0.5">Retirada de Pallets Disponíveis</p>
                  </div>
                </div>
                <button onClick={() => setIsOutflowModalOpen(false)} className="text-[#133020]/30 hover:text-[#133020] transition-colors"><X size={20} /></button>
              </div>

              <div className="p-8 space-y-6 bg-[#FAFAFA]">
                <div className="space-y-4">
                  <label className="text-[10px] font-bold text-[#133020]/50 uppercase tracking-widest block">Quantidade de Saída</label>
                  <div className="flex items-center gap-4">
                    <button onClick={() => setOutflowQty(Math.max(0, outflowQty - 10))} className="p-4 bg-white border border-[#133020]/10 rounded-2xl text-[#133020]/40 hover:bg-red-50 hover:text-red-500 hover:border-red-100 transition-all"><Minus size={20} /></button>
                    <input 
                      type="number" 
                      value={outflowQty}
                    ...
                    />
                    <button onClick={() => setOutflowQty(outflowQty + 10)} className="p-4 bg-white border border-[#133020]/10 rounded-2xl text-[#133020]/40 hover:bg-emerald-50 hover:text-emerald-600 hover:border-emerald-100 transition-all"><Plus size={20} /></button>
                  </div>
                </div>

                <div className="space-y-4">
                  <label className="text-[10px] font-bold text-[#133020]/50 uppercase tracking-widest block">Descrição / Motivo</label>
                  <textarea 
                    value={outflowDesc}
                    onChange={(e) => setOutflowDesc(e.target.value)}
                    className="w-full bg-white border border-[#133020]/10 rounded-2xl px-5 py-4 text-sm font-medium focus:ring-2 focus:ring-[#327039] outline-none transition-all min-h-[120px] resize-none text-[#133020]"

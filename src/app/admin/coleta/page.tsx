...
export default function AdminColetaPage() {
  ...
  const handleSaveColeta = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const coletaData = {
      ...
      cliente_id: "pce" // Fixo conforme regra de negócio atual
    };
    ...
    try {
      ...
      console.error(err);
      alert("Erro ao salvar a coleta: " + err.message);
    } finally {
    ...
    <div className="min-h-screen bg-[#FAFAFA] text-text-dark pb-20">
          ...
          <div className="flex justify-between items-center h-16 sm:h-20">
            <div className="flex items-center gap-4">
              ...
              <nav className="hidden md:flex items-center gap-1 ml-6">
                <Link href="/admin/lotes" className="px-4 py-2 text-text-dark/40 hover:text-text-dark/60 hover:bg-gray-50 rounded-lg text-xs font-bold transition-all">
                  Lotes
                </Link>
                <Link href="/admin/coleta" className="px-4 py-2 bg-brand-cyan/5 text-brand-cyan rounded-lg text-xs font-bold">
                  Coletas
                </Link>
                <Link href="/admin/relatorios" className="px-4 py-2 text-text-dark/40 hover:text-text-dark/60 hover:bg-gray-50 rounded-lg text-xs font-bold transition-all">
                  Relatórios
                </Link>
              </nav>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 pt-10">
        <div className="flex justify-between items-end mb-8">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-text-dark">Coletas PCE Logística</h1>
            <p className="text-text-dark/50 text-sm mt-1">Registro de entrada de material bruto e pallets para triagem.</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="py-32 text-center bg-white rounded-3xl border border-brand-pink/20 card-shadow">
            ...
            <p className="text-sm text-text-dark/40 max-w-xs mx-auto mt-2">Inicie o registro das entradas de material bruto para controle de estoque.</p>
            <button 
              onClick={handleOpenCreate}
              className="mt-6 px-6 py-3 bg-brand-cyan/10 text-brand-cyan rounded-xl font-bold text-xs hover:bg-brand-cyan/20 transition-all"
            >
              ...
              {coletas.map((coleta) => (
                ...
                >
                  <div className="flex justify-between items-start mb-6">
                    <div className="w-12 h-12 bg-bg-primary rounded-2xl flex items-center justify-center text-brand-cyan group-hover:bg-brand-cyan/5 transition-colors">
                      <Truck size={24} />
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => handleOpenEdit(coleta)} className="p-2 text-text-dark/30 hover:text-brand-cyan hover:bg-brand-cyan/5 rounded-lg transition-all">
                        <Edit3 size={16} />
                      </button>
                      <button onClick={() => handleDeleteColeta(coleta.id)} className="p-2 text-text-dark/30 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="pt-4 border-t border-brand-pink/10 grid grid-cols-2 gap-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] font-semibold text-text-dark/60 truncate" title="NF Saída PCE">
                          {coleta.nf_saida_pce || "Sem NF"}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        <User size={14} className="text-text-dark/20" />
                        <span className="text-[11px] font-semibold text-text-dark/60 truncate" title="Motorista">
                          {coleta.motorista || "Não informado"}
                        </span>
                      </div>
                    </div>

                    {coleta.observacao && (
                      <p className="text-[10px] text-text-dark/40 italic line-clamp-2 mt-2 bg-gray-50 p-2 rounded-lg">
                        "{coleta.observacao}"
                      </p>
                    )}
                  </div>
            ...
            >
              <div className="px-8 py-6 border-b border-brand-pink/10 flex justify-between items-center bg-white sticky top-0 z-10">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="font-bold text-lg">{editingColeta ? "Editar Registro" : "Nova Coleta de Material"}</h3>
                </div>
              ...
              <form onSubmit={handleSaveColeta} className="p-8 space-y-5 overflow-y-auto max-h-[70vh]">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div className="space-y-1.5">
                    ...
                    <input 
                      name="data_coleta"
                      type="date"
                      defaultValue={editingColeta?.data_coleta}
                      required
                      className="w-full px-4 py-3 bg-bg-primary border border-brand-pink/20 rounded-xl text-sm focus:ring-2 focus:ring-brand-cyan/20 focus:border-brand-cyan transition-all outline-none"
                    />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-text-dark/40 ml-1">Material Bruto (Qtd) *</label>
                    <input 
                      name="quantidade_material_bruto"
                      type="number"
                      min="1"
                      defaultValue={editingColeta?.quantidade_material_bruto}
                      required
                      placeholder="Ex: 450"
                      className="w-full px-4 py-3 bg-bg-primary border border-brand-pink/20 rounded-xl text-sm focus:ring-2 focus:ring-brand-cyan/20 focus:border-brand-cyan transition-all outline-none"
                    />
                ...
                <div className="space-y-1.5">
                  ...
                  <input 
                    name="nf_saida_pce"
                    defaultValue={editingColeta?.nf_saida_pce}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div className="space-y-1.5">
                    ...
                    <input 
                      name="motorista"
                      defaultValue={editingColeta?.motorista}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input 
                      name="caminhao"
                      defaultValue={editingColeta?.caminhao}
                      placeholder="ABC-1234"
                      className="w-full px-4 py-3 bg-bg-primary border border-brand-pink/20 rounded-xl text-sm focus:ring-2 focus:ring-brand-cyan/20 focus:border-brand-cyan transition-all outline-none uppercase"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea 
                    name="observacao"
                    defaultValue={editingColeta?.observacao}
                    className="w-full px-4 py-3 bg-bg-primary border border-brand-pink/20 rounded-xl text-sm focus:ring-2 focus:ring-brand-cyan/20 focus:border-brand-cyan transition-all outline-none min-h-[100px] resize-none"
                    placeholder="Informações adicionais da carga..."
                  />
                </div>
                ...
                <div className="pt-4 flex gap-3 sticky bottom-0 bg-white">
                  <button 
                    type="button"
                    onClick={() => setIsModalOpen(false)}
                    className="flex-1 px-6 py-3 border border-gray-200 rounded-xl text-xs font-bold hover:bg-gray-50 transition-all text-text-dark/60"
                  >
                    Cancelar
                  </button>
                  <button 
